...
# References to model:
basefolder = 'imgs_h36m_act_all_820/'
# basefolder = './'
joint_status = ['no_joints','joints']

basename = 'epoch' + str(FLAGS.max_epoch-1) + Task + filename + date + joint_status[int(FLAGS.joints)]+'queuemodel.ckpt'
...
direct = 'temp'+Task+joint_status[int(FLAGS.joints)]+'inv_sampleaesthetic'
...
for i,noiseinst1 in enumerate(noiseranges):
    for j,noiseinst2 in enumerate(noiseranges):
        ...
        reconstruct = sess.run(sampled_tensor,feed_dict={injected_tensor:all_activations,y_tensor:y_tensor_all[FLAGS.batch_size*batch:FLAGS.batch_size*batch+FLAGS.batch_size,:],e_tensor:noise})
        img_reconstruct = reconstruct.reshape([FLAGS.batch_size,FLAGS.Imsizex,FLAGS.Imsizey,3])
        axarray[i,j].imshow(img_reconstruct[50,:,:,:])
        axarray[i,j].axis('off')
plt.savefig('Noise_compare_noiserelevant_upweight.png')
