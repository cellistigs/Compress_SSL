...
direct = 'temp'+Task+joint_status[int(FLAGS.joints)]+'inv_sampleshuffle'
...
#         if FLAGS.joints:
#             frame = sess.run(sampled_tensor,feed_dict={input_tensor:imgs_batch,y_tensor:y_tensor_all[FLAGS.batch_size*rand_ind:FLAGS.batch_size*rand_ind+FLAGS.batch_size,:],e_tensor:noise})
#         else:
#             frame = sess.run(frame,feed_dict={input_tensor:imgs_batch,e_tensor:noise})
...
np.save(direct+"/"+'all_residuals',residuals)
