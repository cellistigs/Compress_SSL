...
## We need a axiswise interpolation function across each column
def interp_multi(x,xp,fp):
    ## Find the number of columns in the data:
    columns = fp.shape[1]
    allcolumns = []
    for column in range(columns):
        interpd = np.interp(x,xp,fp[:,column])
        allcolumns.append(interpd)
    ## Now stack these, and return as columns again:
    final = np.vstack(allcolumns).T
    return final
...
# References to model:
basefolder = 'imgs_priorKL/'
# basefolder = './'
joint_status = ['no_joints','joints']

basename = 'epoch' + str(FLAGS.max_epoch-5) + Task + filename + date + joint_status[int(FLAGS.joints)]+'queuemodel.ckpt'
...
injected_tensor = graph.get_tensor_by_name('model_g_1/hidden_input:0')
...
# PCA_done = 0
# batches = 1
noiseranges = np.linspace(-2,2,5)

hidden = []

fig,axarray = plt.subplots(5,5,figsize = (20,20))
...
if FLAGS.joints:
    all_activations = sess.run(hidden_activations,feed_dict={input_tensor:imgs_batch,y_tensor:y_tensor_all[FLAGS.batch_size*batch:FLAGS.batch_size*batch+FLAGS.batch_size,:],e_tensor:noise})
else:
...
## We want to also consider a linear interpolation of the hidden states. Let's consider
## a linear interpolation with different scales of resolution.
skiprange = [2,5,10,20,40] ## how many entries to skip:
interps = [all_activations]
for skip in skiprange:
    xindices = np.arange(FLAGS.batch_size)
    xaxis = xindices[::skip]
    sparse_activations = mean_activations[xaxis,:]
    interpolated = interp_multi(xindices,xaxis,sparse_activations)
    interps.append(np.concatenate((interpolated,all_activations[:,FLAGS.hidden_size:]),axis = 1))
# It seems to me that the two most important directions of variation and
# the two least important directions of variation in the mean signal would be interesting
# to examine in the context of noise directions.
# pca_analysis = PCA(n_components = 2)
# pca_analysis.fit(mean_activations)
# pcs = pca_analysis.components_[-2:,:]*np.sqrt(FLAGS.hidden_size) ## the expected norm of a Gaussian vector is N, so we should normalize to that.

# for frame in range(FLAGS.batch_size):
all_reconstructs = []
for interpolated in interps:
    # noise = np.atleast_2d(np.concatenate((noiseinst1*pcs[0,:]+noiseinst2*pcs[1,:],np.zeros(FLAGS.domain_size+2))))
    # noise = np.tile(noise,(FLAGS.batch_size,1))
    noise = np.zeros((FLAGS.batch_size,FLAGS.hidden_size+FLAGS.domain_size+2))
    ## Now inject this into the rest of the network using the placeholder_with_default configuration you know how to use now.
    reconstruct = sess.run(sampled_tensor,feed_dict={injected_tensor:interpolated,y_tensor:y_tensor_all[FLAGS.batch_size*batch:FLAGS.batch_size*batch+FLAGS.batch_size,:],e_tensor:noise})
    img_reconstruct = reconstruct.reshape([FLAGS.batch_size,FLAGS.Imsizex,FLAGS.Imsizey,3])
    all_reconstructs.append(img_reconstruct)

for frameind in range(FLAGS.batch_size):
    fig,ax = plt.subplots(3,2,figsize=(20,20))
    for i in range(3):
        for j in range(2):
            img_reconstruct = all_reconstructs[i+j*3]
            ax[i,j].imshow(img_reconstruct[frameind,:,:,:])
            ax[i,j].axis('off')
    plt.savefig(direct+'/'+'interpcompare%03d.png'%(frameind))
os.chdir(direct)
subprocess.call(['ffmpeg', '-framerate', str(60/5), '-i', 'interpcompare%03d.png', '-r', '30','Video_invsample'+'.mp4'])
...
os.chdir("../")
        ...
        # mean_activations
        # pca_analysis = PCA(n_components = 2)
        # data_reduced = pca_analysis.fit_transform(mean_activations)
        # pcs = pca_analysis.components_
        # print(np.linalg.norm(components_,axis = 1),'component lengths')
...
#     batch = basebatch
#     imgs_batch = np.zeros((FLAGS.batch_size,Imsize*Imsize*3))
#     for index in range(FLAGS.batch_size):
#         image=img_as_ubyte(clip.get_frame((batch*400+index)*1./clip.fps))
#         # Preprocessing:
#         img_resize = imresize(image,[Imsize,Imsize])
#         ## now normalize to 0-1 range:
#         img_normalized = img_resize/255.
#         ## Flatten:
...
#     noise = np.zeros((FLAGS.batch_size,FLAGS.hidden_size+FLAGS.domain_size+2))
#     if FLAGS.joints:
#         reconstruction = sess.run(sampled_tensor,feed_dict={input_tensor:imgs_batch,y_tensor:y_tensor_all[FLAGS.batch_size*batch:FLAGS.batch_size*batch+FLAGS.batch_size,:],e_tensor:noise})
#     else:
#         reconstruction = sess.run(sampled_tensor,feed_dict={input_tensor:imgs_batch,e_tensor:noise})
#     img_reconstruct = reconstruction.reshape([FLAGS.batch_size,Imsize,Imsize,3])
#
...
#             gs = gridspec.GridSpec(2,3,width_ratios=[0.5,1,1],height_ratios=[3,1])
#             axlist = []
...
#             residual = img_reconstruct[z,:,:,:]-imgs_batch[z,:].reshape(Imsize,Imsize,3)
#
#             ax00.imshow(residual)
#             ax00.set_title('Residual')
#             ax01.imshow(imgs_batch[z,:].reshape(Imsize,Imsize,3))
...
#             ax10.imshow(img_reconstruct[z,ymin:ymax,xmin:xmax,:])
...
#             ax10.set_title('Zoom Paw Reconstruction')
